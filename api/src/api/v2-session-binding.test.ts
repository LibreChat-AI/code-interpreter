...
beforeAll(async () => {
    packageDir = await fsp.mkdtemp(
        path.join(os.tmpdir(), 'headerless-runtime-'),
    );
    await fsp.writeFile(
        path.join(packageDir, 'pkg-info.json'),
        JSON.stringify({
...
describe('per-request session binding', () => {
    test('a headerless execute does not inherit the runner session bound by a prior request', async () => {
        ...
        Job.prototype.prime =
            async function primeWithoutFilesystem(): Promise<void> {
        ...
        Job.prototype.cleanup =
            async function cleanupWithoutFilesystem(): Promise<void> {};
    ...
    test('a different session receives a typed conflict so the control plane can recycle the runner', async () => {
        ...
        try {
            ...
            expect(getBoundSessionWorkspace()?.runtimeSessionId).toBe(
                'rt_bound_once',
            );
        } finally {
            Job.prototype.prime = originalPrime;
