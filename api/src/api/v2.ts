...
import {
    getLatestRuntimeMatchingLanguageVersion,
    getRuntimes,
} from '../runtime';
...
function existingDestinationConflictMessage(
    existing: string,
    destination: string,
): string {
...
export function validateExecuteFiles(files: TFile[]): void {
    if (files.length > config.max_input_files) {
        throw {
            message: `files cannot contain more than ${config.max_input_files} destinations`,
        };
    }
    ...
    for (const [i, value] of files.entries()) {
        ...
        if (file.id !== undefined && !byRef) {
            throw {
                message: `files[${i}].id must be a non-empty string if provided`,
            };
        }
        if (byRef) {
            if (
                ...
                throw {
                    message: `files[${i}].storage_session_id is required as a non-empty string for file refs`,
                };
            }
        } else if (
            file.storage_session_id !== undefined ||
        ...
        if (
            ...
            throw {
                message: `files[${i}].encoding must be base64, hex, or utf8 if provided`,
            };
        }
        if (
            file.entity_id !== undefined &&
            typeof file.entity_id !== 'string'
        ) {
            throw {
                message: `files[${i}].entity_id must be a string if provided`,
            };
        }
        if (
            file.input_cache_key !== undefined &&
            ...
            throw {
                message: `files[${i}].input_cache_key must be a 64-character lowercase hex digest`,
            };
        }
        ...
        if (conflict) {
            throw {
                message: existingDestinationConflictMessage(
                    conflict,
                    destination,
                ),
            };
...
export function sanitizeEnvVars(
    ...
    for (const [key, value] of Object.entries(raw)) {
        ...
        if (totalBytes + entryBytes > MAX_ENV_VAR_BYTES) {
            throw new Error(
                `env_vars exceeds maximum total size of ${MAX_ENV_VAR_BYTES} bytes`,
            );
        }
...
export function authorizeToolCallSocket(
    ...
    const manifestHasBodyHash =
        typeof manifest?.execute_body_sha256 === 'string' &&
        manifest.execute_body_sha256 !== '';

    if (requested) {
        if (
            options.allowUnsignedLocalToolCallSocket === true &&
            manifest === undefined
        ) {
            return true;
        }
...
function getJob(
    ...
    if (
        body.tool_call_socket !== undefined &&
        typeof body.tool_call_socket !== 'boolean'
    ) {
    ...
    if (!hasRunnableSource(files, rt.language)) {
        throw {
            message: 'files must include at least one runnable source file',
        };
    }
    ...
    if (binding) {
        ...
        if (session.dirtyReason) {
            throw {
                ...
                code: 'session_workspace_dirty',
                message:
                    'Session workspace must be restored before another execute',
            };
        }
    }
...
function validateConstraints(body: ExecuteRequestBody, rt: Runtime): void {
    ...
    for (const constraint of constraints) {
        for (const type of types) {
            ...
            if (value > configured) {
                throw {
                    message: `${key} cannot exceed the configured limit of ${configured}`,
                };
            }
            if (value < 0) {
...
router.use((req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.path === '/session/restore' || req.path === '/session/inputs')
        return next();
    if (!req.headers['content-type']?.startsWith('application/json')) {
        return res
            .status(415)
            .json({ message: 'requests must be of type application/json' });
    }
    next();
});
...
router.post(
    '/execute',
    express.json({ limit: config.execute_body_limit }),
    async (req: Request, res: Response) => {
        ...
        let metricsOutcome: Parameters<
            typeof recordSandboxExecution
        >[0]['outcome'] = 'execution_error';
        ...
        try {
            ...
            if (config.require_execution_manifest) {
                try {
                    verifiedManifest = verifyExecuteRequestManifest({
                        headerValue: tokenFromBodyOrHeader(
                            req.body,
                            'execution_manifest',
                            req.header(EXECUTION_MANIFEST_HEADER),
                        ),
                        publicKey: config.execution_manifest_public_key,
                        secret: config.execution_manifest_secret,
                        body: req.body,
                        bodyHashRequiredAfterSeconds:
                            config.execution_manifest_body_hash_required_after_seconds,
                    });
                } catch (error) {
                    ...
                    logger.error(
                        { err: error },
                        'Execution manifest validation failed unexpectedly',
                    );
                    return res.status(500).json({
                        message: 'Execution manifest validation failed',
                    });
                }
            }

            try {
                toolCallSocketEnabled = authorizeToolCallSocket(
                    req.body,
                    verifiedManifest,
                    {
                        legacyClaimGraceUntilSeconds:
                            config.tool_call_socket_legacy_claim_grace_until_seconds,
                        allowUnsignedLocalToolCallSocket:
                            !config.hardened_sandbox_mode &&
                            !config.require_execution_manifest,
                    },
                );
            } catch (error) {
                metricsOutcome = 'manifest_error';
                if (error instanceof ExecutionManifestError) {
                    ...
                    logger.warn(
                        { reason: error.reason, status },
                        'Rejected sandbox request by tool-call socket manifest scope',
                    );
            ...
            try {
                job = getJob(
                    req.body,
                    tokenFromBodyOrHeader(
                        req.body,
                        'egress_grant',
                        req.header(EGRESS_GRANT_HEADER) ?? undefined,
                    ),
                    toolCallSocketEnabled,
                    verifiedManifest?.principal_source ===
                        SYNTHETIC_PRINCIPAL_SOURCE,
            ...
            try {
                ...
                if (result.files && result.files.length > 0) {
                    ...
                    const uploaded = await withSpan(
                        'codeapi.sandbox.upload_generated_files',
                        {
                            'codeapi.language': job.runtime.language,
                        },
                ...
                if (
                    primeCompleted &&
                    job?.markSessionDirty(
                        'execution failed after input priming',
                    )
                ) {
                    metricsOutcome = 'execution_error';
                    logger.error(
                        { job: job.uuid, err: error },
                    ...
                    logger.error(
                        {
                            job: job?.uuid,
                            err: error,
                            safeError: safeError.body.error,
                        },
...
function bindSessionFromHeader(req: Request): SessionBindFailure | null {
    let binding;
    try {
        binding = parseSessionBindingFromHeader(
            req.headers[RUNTIME_SESSION_ID_HEADER],
        );
    } catch (error) {
        ...
        return {
            ...
            body: {
                message:
                    error instanceof Error
                        ? error.message
                        : 'Invalid runtime session header',
            },
        };
    }
    if (!binding) {
        return {
            status: 409,
            body: { message: 'Missing runtime session header' },
        };
    }
...
router.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            const refs = (req.body as { refs?: Array<{ cache_key?: unknown }> })
                ?.refs;
            if (!Array.isArray(refs)) {
                return res
                    .status(400)
                    .json({ message: 'refs must be an array' });
            }
            ...
            for (const ref of refs) {
                if (
                    ...
                    return res.status(400).json({
                        message: 'each ref requires a valid cache_key',
                    });
                }
                if (seen.has(ref.cache_key)) {
                    return res.status(400).json({
                        message: 'refs contains a duplicate cache_key',
                    });
                }
        ...
        try {
            ...
            if (expandedHeader !== undefined) {
                if (!/^(0|[1-9][0-9]*)$/.test(expandedHeader)) {
                    return res
                        .status(400)
                        .json({ message: 'invalid expanded input byte count' });
                }
            ...
            return res
                .status(500)
                .json({ message: 'session input delivery failed' });
        }
    },
);
