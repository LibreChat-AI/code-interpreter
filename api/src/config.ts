...
const requireExecutionManifest =
    (process.env.SANDBOX_REQUIRE_EGRESS_MANIFEST ??
        (egressGatewayUrl ? 'true' : 'false')) === 'true';
...
export function legacyPackagesDirectory(
    raw: string | undefined,
): string | undefined {
...
export const config = {
    ...
    packages_directory:
        cleanDirectory(process.env.SANDBOX_PACKAGES_DIRECTORY) ??
        legacyPackagesDirectory(process.env.SANDBOX_DATA_DIRECTORY) ??
        '/pkgs',
    disable_networking:
        (process.env.SANDBOX_DISABLE_NETWORKING ?? 'true') === 'true',
    use_cgroupv2: (process.env.SANDBOX_USE_CGROUPV2 ?? 'true') === 'true',
    allowed_local_network_port: Number(
        process.env.SANDBOX_ALLOWED_LOCAL_NETWORK_PORT ?? 0,
    ),
    ...
    compile_memory_limit: Number(
        process.env.SANDBOX_COMPILE_MEMORY_LIMIT ?? -1,
    ),
    ...
    session_workspace_enabled:
        (process.env.SANDBOX_SESSION_WORKSPACE_ENABLED ?? 'false') === 'true',
    ...
    workspace_reaper_max_age_seconds: safeInt(
        process.env.SANDBOX_WORKSPACE_REAPER_MAX_AGE_SECONDS,
        3600,
    ),
    ...
    nsjail_config:
        process.env.NSJAIL_CONFIG ?? '/sandbox_api/config/sandbox.cfg',
    ...
    execution_manifest_body_hash_required_after_seconds:
        sandboxStartedAtSeconds +
        safeInt(
            process.env
                .SANDBOX_EXECUTION_MANIFEST_BODY_HASH_LEGACY_GRACE_SECONDS,
            600,
            0,
        ),
    tool_call_socket_legacy_claim_grace_until_seconds:
        sandboxStartedAtSeconds +
        safeInt(
    ...
    limit_overrides: parseLimitOverrides(
        process.env.SANDBOX_LIMIT_OVERRIDES ?? '{}',
    ),
};
