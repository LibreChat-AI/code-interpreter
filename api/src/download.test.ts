...
afterAll(() => {
    ...
    (config as { egress_gateway_url: string }).egress_gateway_url =
        originalEgressGatewayUrl;
...
describe('downloadAndWriteFile / RFC 5987 round-trip', () => {
    it('writes a nested-path artifact at the encoded location', async () => {
        ...
        routes.set(
            `/sessions/${encodeURIComponent(file.storage_session_id!)}/objects/${encodeURIComponent(file.id!)}`,
            {
                status: 200,
                contentDisposition:
                    "attachment; filename*=UTF-8''proj%2Fnotes.txt",
        ...
        expect((await fsp.stat(path.dirname(expectedFull))).mode & 0o777).toBe(
            expectedWritableMode(SANDBOX_DIR_MODE),
        );
        expect((await fsp.stat(expectedFull)).mode & 0o777).toBe(
            expectedWritableMode(SANDBOX_FILE_MODE),
        );
    });
    ...
    it('uses the egress gateway URL and grant header when configured', async () => {
        ...
        routes.set(
            `/sessions/${encodeURIComponent(file.storage_session_id!)}/objects/${encodeURIComponent(file.id!)}`,
            {
                ...
                onRequest(req) {
                    sawGrantHeader =
                        req.headers.get('x-codeapi-egress-grant') ===
                        'opaque-grant';
                    sawInternalHeader = req.headers.has(
                        'x-codeapi-internal-token',
                    );
                },
            },
        );
        ...
        expect(
            await fsp.readFile(path.join(tmpDir, 'gateway.txt'), 'utf8'),
        ).toBe('gateway bytes');
    });
    ...
    it('keeps read-only downloaded inputs non-writable to the sandbox owner', async () => {
        ...
        expect(
            (await fsp.stat(path.join(tmpDir, 'readonly.txt'))).mode & 0o777,
        ).toBe(SANDBOX_READONLY_FILE_MODE);
    });
    ...
    it('resolves concurrent header destinations without provisional-name false conflicts', async () => {
        ...
        routes.set(
            `/sessions/${encodeURIComponent(renamed.storage_session_id!)}/objects/${encodeURIComponent(renamed.id!)}`,
            {
                status: 200,
        ...
        routes.set(
            `/sessions/${encodeURIComponent(replacement.storage_session_id!)}/objects/${encodeURIComponent(replacement.id!)}`,
            {
                status: 200,
        ...
        try {
            ...
            expect(
                await fsp.readFile(
                    path.join(submissionDir, 'actual.txt'),
                    'utf8',
                ),
            ).toBe('renamed bytes');
            expect(
                await fsp.readFile(
                    path.join(submissionDir, 'vacated.txt'),
                    'utf8',
                ),
    ...
    it('rejects concurrent refs that resolve to the same destination before either can overwrite', async () => {
        ...
        routes.set(
            `/sessions/${encodeURIComponent(slower.storage_session_id!)}/objects/${encodeURIComponent(slower.id!)}`,
            {
                status: 200,
        ...
        routes.set(
            `/sessions/${encodeURIComponent(faster.storage_session_id!)}/objects/${encodeURIComponent(faster.id!)}`,
            {
                status: 200,
                contentDisposition: 'attachment; filename="same.txt"',
                body: 'faster bytes',
        ...
        const job = makeJob(
            [slower, faster],
            sessionWorkspaceAt(tmpDir, 'rt_concurrent_same_destination', () => {
                dirty = true;
            }),
        );
        const originalPrimeConcurrency = config.prime_concurrency;
        ...
        try {
            ...
            expect(
                await fsp.readFile(path.join(tmpDir, 'same.txt'), 'utf8'),
            ).toBe('faster bytes');
        } finally {
            if (deadlockTimer) clearTimeout(deadlockTimer);
            config.prime_concurrency = originalPrimeConcurrency;
    ...
    it('fails when the server keeps 404-ing past the retry cap (no phantom write)', async () => {
        ...
        await expect(
            fsp.access(path.join(tmpDir, 'should-not-exist.txt')),
        ).rejects.toThrow();
    });
    ...
    it('rejects a server-supplied filename that escapes the submission dir', async () => {
        ...
        routes.set(
            `/sessions/${encodeURIComponent(file.storage_session_id!)}/objects/${encodeURIComponent(file.id!)}`,
            {
                status: 200,
                contentDisposition:
                    "attachment; filename*=UTF-8''..%2F..%2Fescape.txt",
        ...
        await expect(
            fsp.access(path.join(parent, 'escape.txt')),
        ).rejects.toThrow();
    });
});
