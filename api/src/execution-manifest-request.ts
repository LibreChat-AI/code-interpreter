...
function assertManifestBodyHashMatches(
    ...
    if (!manifest.execute_body_sha256) {
        ...
        throw new ExecutionManifestError(
            'scope_mismatch',
            'Execution manifest body hash does not match request',
        );
    }
...
export function collectExecuteRequestInputFiles(
    body: ExecuteRequestBody,
    ...
    return files
        .map((file, index) => {
            ...
            if (!storageId) {
                throw new ExecutionManifestError(
                    'scope_mismatch',
                    'Execution manifest input file scope does not match request',
                );
            }
            return {
                id: file.id,
...
export function assertManifestMatchesExecuteRequest(
    ...
    if (!outputSessionId || manifest.output_session_id !== outputSessionId) {
        throw new ExecutionManifestError(
            'scope_mismatch',
            'Execution manifest output session does not match request',
        );
    }

    const requestFiles = collectExecuteRequestInputFiles(body);
    const manifestFiles = [...manifest.input_files].sort(
        (a, b) =>
            a.session_id.localeCompare(b.session_id) ||
            a.id.localeCompare(b.id) ||
            a.name.localeCompare(b.name),
    ...
    const expectedReadSessions = Array.from(
        new Set(requestFiles.map(file => file.session_id)),
    ).sort();
    const manifestReadSessions = [...manifest.read_sessions].sort();
    if (!stringArraysEqual(expectedReadSessions, manifestReadSessions)) {
        throw new ExecutionManifestError(
            'scope_mismatch',
            'Execution manifest read sessions do not match request',
        );
    }
...
export function verifyExecuteRequestManifest(args: {
    ...
    if (!args.headerValue) {
        throw new ExecutionManifestError(
            'missing_header',
            `${EXECUTION_MANIFEST_HEADER} is required`,
        );
    }
    const manifest = verifyExecutionManifestWithKey(
        args.headerValue,
        {
