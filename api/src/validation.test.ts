...
describe('checkPathShape / isValidPathShape', () => {
    ...
    it('rejects paths at exactly max_nesting_depth + 1 segments (boundary)', () => {
        const exactBoundary =
            Array(MAX_DEPTH).fill('x').join('/') + '/file.txt';
...
describe('validateFilePath', () => {
    ...
    it('accepts well-formed relative paths', () => {
        ...
        expect(() =>
            validateFilePath('dir/file.txt', submissionDir),
        ).not.toThrow();
    });

    it('rejects non-canonical paths (.., ./, //)', () => {
        expect(() =>
            validateFilePath('a/../a/file.txt', submissionDir),
        ).toThrow(ValidationError);
        expect(() => validateFilePath('dir/./file.txt', submissionDir)).toThrow(
            ValidationError,
        );
        expect(() => validateFilePath('dir//file.txt', submissionDir)).toThrow(
            ValidationError,
    ...
    it('rejects directory-style paths with a trailing slash', () => {
        expect(() => validateFilePath('dir/', submissionDir)).toThrow(
            /trailing|directory/,
        );
        expect(() => validateFilePath('a/b/', submissionDir)).toThrow(
            /trailing|directory/,
        );
    });

    it('rejects empty names and bare dot', () => {
        expect(() => validateFilePath('', submissionDir)).toThrow(
            ValidationError,
        );
        expect(() => validateFilePath('.', submissionDir)).toThrow(
            ValidationError,
    ...
    it('rejects path traversal with .. segments', () => {
        expect(() => validateFilePath('../etc/passwd', submissionDir)).toThrow(
            ValidationError,
        );
        expect(() => validateFilePath('a/../../escape', submissionDir)).toThrow(
            ValidationError,
        );
        expect(() =>
            validateFilePath('a/b/../../../escape', submissionDir),
        ).toThrow(ValidationError);
    });
    ...
    it('rejects paths exceeding length limits with a specific message', () => {
        const tooLong = 'a'.repeat(MAX_LEN + 10);
        expect(() => validateFilePath(tooLong, submissionDir)).toThrow(
            /maximum length/,
        );
    });
    ...
    it('rejects paths exceeding nesting depth with a specific message', () => {
        const tooDeep =
            Array(MAX_DEPTH + 1)
                .fill('x')
                .join('/') + '/file.txt';
        expect(() => validateFilePath(tooDeep, submissionDir)).toThrow(
            /nesting depth/,
        );
    });
...
describe('isValidFilePath', () => {
    ...
    it('returns false for empty or shape-violating paths', () => {
        ...
        expect(isValidFilePath('a'.repeat(MAX_LEN + 10), submissionDir)).toBe(
            false,
        );
    });
});
