...
export function validateFilePath(name: string, submissionDir: string): void {
    ...
    if (path.posix.normalize(name) !== name) {
        throw new ValidationError(
            `File path "${name}" must be in canonical form (no "..", "./", or "//" segments)`,
        );
    }
    ...
    if (name.endsWith('/')) {
        throw new ValidationError(
            `File path "${name}" must not end with "/" (directory paths are not valid file names)`,
        );
    }
    ...
    if (rel === '..' || rel.startsWith('..' + path.sep)) {
        throw new ValidationError(
            `File path "${name}" escapes parent directory`,
        );
    }
}
