...
export async function readRepositoryInstructions(root: string): Promise<RepositoryInstructionSnapshot | undefined> {
  ...
  try {
    ...
    if (!opened.isFile() || isAbsolute(offset) || offset === '..' || offset.startsWith('../') ||
      opened.dev !== current.dev || opened.ino !== current.ino) return;
    ...
    const content = decoder.decode(buffer.subarray(0, Math.min(length, REPOSITORY_INSTRUCTION_MAX_BYTES)), { stream: truncated });
    if (content.includes('\0')) return;
    return {
      descriptor: { path, bytes: Buffer.byteLength(content), sha256: createHash('sha256').update(content).digest('hex'), truncated },
      content,
    };
  } catch {
    return;
