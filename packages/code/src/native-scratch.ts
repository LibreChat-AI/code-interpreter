...
const lib = POSIX_PLATFORMS.has(process.platform)
    ? koffi.load(null)
    : undefined;
const openat = lib?.func(
    'int openat(int dirfd, const char *path, int flags, uint32_t mode)',
);
...
function restoreEntryMode(directoryFd: number, name: string): boolean {
    ...
    throw new Error(
        `Descriptor-relative scratch chmod failed with errno ${koffi.errno()}`,
    );
}
...
function openDirectoryAt(
    ...
    throw new Error(
        `Descriptor-relative scratch open failed with errno ${koffi.errno()}`,
    );
}
...
function closeDirectory(fd: number): void {
    ...
    if (closeFd!(fd) !== 0) {
        throw new Error(
            `Descriptor-relative scratch close failed with errno ${koffi.errno()}`,
        );
    }
}
...
async function openRelativeDirectory(
    ...
    try {
        for (const component of components) {
            ...
            currentFd = childFd;
            await repairDirectory(
                currentFd,
                `scratch directory ${components.join('/')}`,
            );
        }
        return currentFd;
    } catch (error) {
...
export async function restoreScratchTraversal(
    ...
    const consumeComponentVisit = () => {
        ...
        if (componentVisits > MAX_SCRATCH_COMPONENT_VISITS) {
            throw new Error(
                'Native sandbox scratch cleanup exceeded its work limit',
            );
        }
    };
        ...
        try {
            ...
            for await (const entry of directory) {
                ...
                if (entriesInspected > MAX_SCRATCH_ENTRIES) {
                    throw new Error(
                        'Native sandbox scratch cleanup exceeded its entry limit',
                    );
                }
                if (!entry.isDirectory()) continue;
                if (components.length >= MAX_SCRATCH_DEPTH) {
                    throw new Error(
                        'Native sandbox scratch cleanup exceeded its depth limit',
                    );
                }
                if (pending.length >= MAX_SCRATCH_DIRECTORIES) {
                    throw new Error(
                        'Native sandbox scratch cleanup exceeded its directory limit',
                    );
                }
