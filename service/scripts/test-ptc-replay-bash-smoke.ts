...
import { extractPendingFromStdout, type LCTool } from '../src/preamble';
import {
    generateBashReplayPreamble,
    generateBashReplayPostamble,
} from '../src/preamble-bash';
...
const preamble = generateBashReplayPreamble({
    executionId: BASH_SMOKE_EXEC_ID,
    tools,
});
...
const extractPending = (s: string) =>
    extractPendingFromStdout(s, BASH_SMOKE_EXEC_ID);
...
`;
    ...
    assert(
        p.pending !== null && p.pending.length === 1,
        'single: one pending call',
    );
    assert(
    ...
    assert(
        Boolean(
            (p.pending?.[0]?.input as { city?: string } | undefined)?.city ===
            'Paris',
        ),
        'single: input.city is Paris',
    );
    assert(
        p.pending?.[0]?.call_id === 'call_001',
        'single: call_id is call_001',
    );
    assert(
        typeof p.pending?.[0]?.input_hash === 'string' &&
            p.pending[0].input_hash.length === 64,
        'single: bash input_hash metadata emitted',
    );
    assert(
        typeof p.pending?.[0]?.call_site === 'string',
        'single: call_site metadata emitted',
    );
}
...
`;
    ...
    assert(
        Boolean(p1.pending?.[0]?.call_id === 'call_001'),
        'seq: first pending is call_001',
    );

    const history1 = {
        call_001: { result: { temperature: 68, city: 'Paris' } },
    };
    const r2 = runBash(assemble(user), history1);
    const p2 = extractPending(r2.stdout);
    assert(
        Boolean(p2.pending?.[0]?.call_id === 'call_002'),
        'seq: second pending is call_002',
    );
    assert(
...
`;
    const history = {
        call_001: { is_error: true, error_message: 'city not found' },
    };
    const r = runBash(assemble(user), history);
    assert(r.exitCode === 1, 'error: exit 1');
    assert(
        r.stderr.includes('city not found'),
        'error: stderr contains message',
    );
    assert(
        !r.stdout.includes('UNREACHABLE'),
        'error: aborted before next line',
    );
}
...
`;
    ...
    assert(
        r.signal !== 'SIGTERM',
        'no_tool_background: did not wait for unrelated job',
    );
...
`;
    ...
    assert(
        p1.pending !== null && p1.pending.length === 1,
        'redirect_pending: pending emitted',
    );
    assert(
        Boolean(
            (p1.pending?.[0]?.input as { city?: string } | undefined)?.city ===
            'Seoul',
        ),
        'redirect_pending: correct input',
    );
    assert(
        !p1.stdout.includes('file='),
        'redirect_pending: aborts before reading file',
    );
...
`;
    ...
    assert(
        p.pending !== null && p.pending.length === 2,
        'parallel_pending: two pending calls emitted as one batch',
    );
    assert(
    ...
    assert(
        !r.stdout.includes('AFTER'),
        'parallel_pending: aborts before post-wait command',
    );
}
...
`;
    ...
    assert(
        p.pending !== null && p.pending.length === 2,
        'parallel_no_wait: background tool calls emitted as one batch',
    );
    assert(
        p.stdout.includes('launched'),
        'parallel_no_wait: user stdout before implicit join preserved',
    );
    assert(
        Boolean(p.pending?.some(call => call.tool_name === 'get_weather')),
...
`;
    ...
    assert(
        p.pending !== null && p.pending.length === 2,
        'assignment_parallel: assignment-prefixed tools batch',
    );
    assert(
        p.stdout.includes('assigned'),
        'assignment_parallel: user stdout before implicit join preserved',
    );
    assert(
        Boolean(p.pending?.some(call => call.tool_name === 'get_weather')),
...
`;
    ...
    assert(
        p.pending !== null && p.pending.length === 2,
        'quoted_assignment_parallel: quoted assignment tools batch',
    );
    assert(
        p.stdout.includes('quoted assignment mid'),
        'quoted_assignment_parallel: intervening stdout preserved',
    );
    assert(
        Boolean(p.pending?.some(call => call.tool_name === 'get_weather')),
...
`;
    ...
    assert(
        p.pending !== null && p.pending.length === 2,
        'time_parallel: time-prefixed tools batch',
    );
    assert(
        p.stdout.includes('time mid'),
        'time_parallel: intervening stdout preserved',
    );
    assert(
        Boolean(p.pending?.some(call => call.tool_name === 'get_weather')),
...
`;
    ...
    assert(
        p.pending !== null && p.pending.length === 2,
        'group_parallel: grouped tool calls batch',
    );
    assert(
        p.stdout.includes('group mid'),
        'group_parallel: intervening stdout preserved',
    );
    assert(
        Boolean(p.pending?.some(call => call.tool_name === 'get_weather')),
...
`;
    ...
    assert(
        r.signal !== 'SIGTERM',
        'unrelated_background: did not wait for unrelated job',
    );
    assert(r.exitCode === 0, 'unrelated_background: exit 0');
    assert(
        p.pending !== null && p.pending.length === 1,
        'unrelated_background: one pending call',
    );
    assert(
        p.stdout.includes('launched unrelated'),
        'unrelated_background: stdout preserved',
    );
    assert(
        Boolean(
            (p.pending?.[0]?.input as { city?: string } | undefined)?.city ===
...
`;
    ...
    assert(
        p.pending !== null && p.pending.length === 1,
        'cmdsub_with_unrelated_job: emits pending immediately',
    );
    assert(
        Boolean(
            (p.pending?.[0]?.input as { city?: string } | undefined)?.city ===
    ...
    assert(
        !r.stdout.includes('AFTER'),
        'cmdsub_with_unrelated_job: aborts before next command',
    );
}
...
`;
    ...
    assert(
        r.signal !== 'SIGTERM',
        'mixed_background_cmdsub: did not wait for unrelated work before emitting',
    );
    assert(r.exitCode === 0, 'mixed_background_cmdsub: exit 0');
    assert(
        p.pending !== null && p.pending.length === 2,
        'mixed_background_cmdsub: emits both pending calls',
    );
    assert(
        Boolean(p.pending?.some(call => call.tool_name === 'get_weather')),
    ...
    assert(
        !p.stdout.includes('SIDE_EFFECT'),
        'mixed_background_cmdsub: aborts before command-substitution side effect',
    );
}
...
`;
    ...
    assert(
        r.signal !== 'SIGTERM',
        'backtick_cmdsub_with_unrelated_job: did not wait for unrelated job',
    );
    assert(r.exitCode === 0, 'backtick_cmdsub_with_unrelated_job: exit 0');
    assert(
        p.pending !== null && p.pending.length === 1,
        'backtick_cmdsub_with_unrelated_job: emits pending immediately',
    );
    assert(
        Boolean(
            (p.pending?.[0]?.input as { city?: string } | undefined)?.city ===
    ...
    assert(
        !r.stdout.includes('AFTER'),
        'backtick_cmdsub_with_unrelated_job: aborts before next command',
    );
}
...
`;
    ...
    assert(
        p2.stdout.includes('calc=2'),
        'bash_hash_replay: second result matched',
    );
    assert(
        Boolean(p2.pending?.[0]?.call_id === 'call_003'),
        'bash_hash_replay: next pending call_id is call_003',
    );
}
...
`;
    ...
    assert(
        p.stdout.includes('calc=2'),
        'parallel_replay: calculate result matched by tool signature',
    );
    assert(
    ...
    assert(
        Boolean(p.pending?.[0]?.call_id === 'call_003'),
        'parallel_replay: next pending call_id is call_003',
    );
    assert(
        Boolean(
            (p.pending?.[0]?.input as { city?: string } | undefined)?.city ===
