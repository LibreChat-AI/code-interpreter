...
function gatewayUrl(path: string): string {
    if (!env.EGRESS_GATEWAY_URL.trim()) {
        throw new Error(
            'EGRESS_GATEWAY_URL is required for hardened sandbox egress',
        );
    }
...
export async function restoreGatewaySandboxResult<
...
>(args: {
    ...
    const response = await withSpan(
        'codeapi.egress_grant.restore_result',
        {
            'http.request.method': 'POST',
        ...
        () =>
            axios.post<{ result: T }>(
                ...
                {
                    result: args.result,
                    egressGrantToken: args.egressGrantToken,
                },
                {
                    headers: injectTraceHeaders(
                        gatewayHeaders(
...
export async function revokeGatewayEgressGrant(args: {
    ...
    if (!args.grantId && !args.egressGrantToken) {
        throw new Error(
            'grantId or egressGrantToken is required to revoke egress grant',
        );
    }
    const path = args.grantId
    ...
    await withSpan(
        ...
        () =>
            axios.post(gatewayUrl(path), body, {
                headers: injectTraceHeaders(
                    gatewayHeaders(
                        { 'Content-Type': 'application/json' },
                        args.isSynthetic,
...
export async function createGatewayPtcCallbackToken(
    args: {
        executionId: string;
        sessionId: string;
    ...
    const response = await withSpan(
        'codeapi.ptc_callback_token.create',
        {
            'http.request.method': 'POST',
        ...
        () =>
            axios.post<{ callbackToken: string }>(
                ...
                args,
                {
                    ...
                    timeout:
                        options.timeoutMs ??
                        env.EGRESS_GATEWAY_REQUEST_TIMEOUT_MS,
                },
            ),
        'CLIENT',
    );
