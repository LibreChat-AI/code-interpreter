...
app.post(
    ...
    async (req, res) => {
        try {
            ...
            const grantId = Array.isArray(req.params.grantId)
                ? req.params.grantId[0]
                : req.params.grantId;
            return await restoreInternalSandboxResult({
                res,
                result: body.result,
    ...
    async (req, res) => {
        try {
            ...
            const grantId = Array.isArray(req.params.grantId)
                ? req.params.grantId[0]
                : req.params.grantId;
            await revokeEgressLedger(grantId, reason);
            return res.status(204).end();
        } catch (error) {
            return sendEgressError(req, res, error);
        }
...
app.put('/sessions/:sessionHandle/objects/:fileId', async (req, res) => {
    let reservedUpload:
        { grant: EgressGrantClaims; fileId: string; bytes: number } | undefined;
    try {
        if (Object.keys(req.query).length > 0) {
            return res.status(400).json({
        ...
        if (reservedUpload) {
            await releaseEgressUpload(reservedUpload).catch(releaseError => {
                logger.error(
                    'Failed to release egress upload reservation after upstream failure',
                    {
                        error: releaseError,
