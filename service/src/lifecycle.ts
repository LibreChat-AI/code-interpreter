    ...
    try {
        if (hasWorkers) {
            ...
            const pauseAndDrain = async (
                ...
                logger.info(
                    `Pausing ${name} worker and waiting for active jobs to drain...`,
                );
                try {
            ...
            // Close workers
            await Promise.all([pyWorker.close(), otherWorker.close()]);
...
export async function startServer(
    ...
    try {
        ...
        app.listen(env.PORT, () => {
            logger.info(
                `[${INSTANCE_ID}] Server is running on port ${env.PORT}`,
            );
            logger.info(
                `[${INSTANCE_ID}] PYTHON_CONCURRENCY: ${env.PYTHON_CONCURRENCY} | OTHER_CONCURRENCY: ${env.OTHER_CONCURRENCY} | JOB_WINDOW: ${env.JOB_WINDOW}`,
            );
        });
...
export async function startApiServer(
    ...
    try {
        ...
        app.listen(env.PORT, () => {
            logger.info(
                `[${INSTANCE_ID}] API Server is running on port ${env.PORT}`,
            );
