...
class LazyLambdaMicrovmSandboxBackend implements SandboxBackend {
    ...
    private load(): Promise<SandboxBackend> {
        this.backendPromise ??= (async (): Promise<SandboxBackend> => {
            const { LambdaMicrovmSandboxBackend } =
                await import('./lambda-microvm');
            const checkpointStore =
                env.SESSION_CHECKPOINTS &&
                env.RUNTIME_SESSION_MODE !== 'stateless'
                    ? new (
                          await import('../runtime-session/checkpoint-store')
            ...
            return new LambdaMicrovmSandboxBackend({
                ...
                clientFactory: async (): Promise<LambdaMicrovmClient> => {
                    const { AwsLambdaMicrovmClient } =
                        await import('../runtime-session/lambda-client-aws');
                    return new AwsLambdaMicrovmClient({
                        region: env.LAMBDA_MICROVM_REGION,
                    });
                },
                config: {
                    ...
                    ingressConnectorArns:
                        env.LAMBDA_MICROVM_INGRESS_CONNECTOR_ARNS,
                    egressConnectorArns:
                    ...
                    authTokenTtlSeconds:
                        env.LAMBDA_MICROVM_AUTH_TOKEN_TTL_SECONDS,
...
export function setSandboxBackendForTests(
    next: SandboxBackend | undefined,
): void {
    backend = next;
}
