...
import {
    ...
    checkpointObjectKey,
    checkpointPrefixFor,
} from '../runtime-session/checkpoint-store';
import {
...
let recordDuringRestore:
    Awaited<ReturnType<typeof readRuntimeSessionRecord>> | undefined;
        ...
        async fetch(req) {
            ...
            if (path === '/api/v2/session/inputs/probe') {
                if (sessionProbeDelayMs > 0) {
                    await new Promise(resolve =>
                        setTimeout(resolve, sessionProbeDelayMs),
                    );
                }
                const refs = (
            ...
            if (path === '/api/v2/session/inputs') {
                ...
                if (sessionFilesStatus === 200) {
                    ...
                    for (const ref of lastProbedRefs)
                        vmInputCache.add(ref.cache_key);
                    if (evictAfterPush) vmInputCache.clear();
                }
                return new Response(
                    JSON.stringify({ stored: lastProbedRefs.length }),
                    {
            ...
            if (path === '/api/v2/execute') {
                ...
                if (executeDelayMs > 0) {
                    await new Promise(resolve =>
                        setTimeout(resolve, executeDelayMs),
                    );
                }
            ...
            if (path === '/api/v2/session/checkpoint') {
                ...
                return new Response(checkpointBlob, {
                    status: 200,
                    headers: { 'Content-Type': 'application/x-gtar' },
                });
            }
            if (path === '/api/v2/session/restore') {
                recordDuringRestore =
                    await readRuntimeSessionRecord('rt_ckpt_1');
...
function config(
    overrides: Partial<LambdaMicrovmBackendConfig> = {},
): LambdaMicrovmBackendConfig {
    return {
...
function fakeClient(): FakeLambdaMicrovmClient {
    return new FakeLambdaMicrovmClient({
        endpointProvider: () => `http://localhost:${server.port}`,
    });
}
...
function payloadBody(): t.PayloadBody {
    return {
        ...
        files: [
            {
                id: 'file_1',
                storage_session_id: 'sess_store_1',
...
function context(
    overrides: Partial<SandboxExecuteContext> = {},
): SandboxExecuteContext {
    return {
        executionId: 'exec_42',
        language: 'python',
...
describe('normalizeMicrovmEndpoint', () => {
    test('prefixes https for bare hosts and keeps explicit schemes', () => {
        expect(normalizeMicrovmEndpoint('abc.lambda-microvm.on.aws')).toBe(
            'https://abc.lambda-microvm.on.aws',
        );
        expect(normalizeMicrovmEndpoint('abc.on.aws/')).toBe(
            'https://abc.on.aws',
        );
        expect(normalizeMicrovmEndpoint('http://localhost:1234')).toBe(
            'http://localhost:1234',
        );
        expect(normalizeMicrovmEndpoint('https://x.on.aws///')).toBe(
            'https://x.on.aws',
        );
    });
});
...
describe('runtime session launch tokens', () => {
    test('uses a deterministic launch namespace and stays within the AWS limit', () => {
        ...
        expect(seed).toBeGreaterThanOrEqual(
            RUNTIME_SESSION_NAMESPACED_GENERATION_MIN,
        );
        expect(
            runtimeSessionLaunchGenerationSeed({ ...cfg, imageVersion: '4' }),
        ).not.toBe(seed);

        const runtimeSessionId = `rt_${'a'.repeat(40)}`;
        const token = runtimeSessionLaunchClientToken(
            runtimeSessionId,
...
describe('LambdaMicrovmSandboxBackend stateless execution', () => {
    test('run -> health -> execute -> terminate happy path', async () => {
        ...
        const runArgs = runCalls[0].args as {
            imageIdentifier: string;
            clientToken?: string;
            maximumDurationSeconds: number;
        };
        expect(runArgs.imageIdentifier).toBe(
            'arn:aws:lambda:us-east-2:1:microvm-image:codeapi',
        );
        expect(runArgs.clientToken).toBe('exec-exec_42');
        expect(runArgs.maximumDurationSeconds).toBe(
            Math.ceil(300_000 / 1_000) + 120,
        );
        ...
        expect(vm.mintedTokens).toContain(
            executeReq?.headers['x-aws-proxy-auth'] as string,
        );
    ...
    test('terminates the VM even when the execute is aborted mid-flight', async () => {
        ...
        const pending = makeBackend(fake).execute(
            request(),
    ...
    test('launch poll timeout surfaces MICROVM_LAUNCH_FAILED and terminates the stuck VM', async () => {
        ...
        try {
            ...
            expect((error as SandboxBackendError).code).toBe(
    ...
    test('shares one launch budget across throttle, RunMicrovm, and control-plane polling', async () => {
        ...
        await expect(
            makeBackend(fake, { launchTimeoutMs: 120 }).execute(
                request(),
                context(),
            ),
        ...
        expect(
            captured.some(request => request.path === '/api/v2/execute'),
        ).toBe(false);
        expect(fake.callsFor('terminateMicrovm')).toHaveLength(1);
    });
    ...
    test('rejects and terminates a RUNNING response returned after the launch deadline', async () => {
        ...
        await expect(
            makeBackend(fake, { launchTimeoutMs: 40 }).execute(
                request(),
                context(),
            ),
        ).rejects.toMatchObject({
    ...
    test('a MicroVM that dies during boot is retried once with a fresh clientToken', async () => {
        ...
        const tokens = runCalls.map(
            call => (call.args as { clientToken?: string }).clientToken,
        );
    ...
    test('the boot-death retry consumes only the first attempt remaining launch budget', async () => {
        ...
        await expect(
            makeBackend(fake, { launchTimeoutMs: 300 }).execute(
                request(),
                context(),
            ),
        ).rejects.toMatchObject({
    ...
    test('a second boot-time death fails the request — single retry only', async () => {
        ...
        await expect(
            makeBackend(fake).execute(request(), context()),
        ).rejects.toMatchObject({
            code: 'MICROVM_LAUNCH_FAILED',
        });
...
describe('LambdaMicrovmSandboxBackend session execution', () => {
    ...
    test('launches a hookless session VM (idlePolicy, no runHookPayload) and stamps the workspace header on execute', async () => {
        ...
        expect(runArgs.clientToken).toBe(
            runtimeSessionLaunchClientToken(
                'rt_session_1',
        ...
        const executeReq = captured.find(c => c.path === '/api/v2/execute');
        expect(executeReq?.headers['x-runtime-session-id']).toBe(
            'rt_session_1',
        );
        ...
        expect(record?.generation).toBeGreaterThanOrEqual(
            RUNTIME_SESSION_NAMESPACED_GENERATION_MIN,
        );
    ...
    test('replays a legacy recorded launch intent after the RunMicrovm response is lost', async () => {
        ...
        await writeRuntimeSessionRecord(
            {
                ...
                hard_deadline_at:
                    launchedAt + cfg.maxDurationSeconds * 1_000 - 60_000,
            },
            lock as string,
        );
        await releaseRuntimeSessionLock('rt_session_1', lock as string);

        const result = await makeBackend(fake).execute(
            request(),
            sessionContext(),
        );
        expect(result).toEqual(EXECUTE_RESPONSE);
        ...
        expect(
            (await readRuntimeSessionRecord('rt_session_1'))?.microvm_id,
    ...
    test('persists and replays the exact launch token after an ambiguous lost response', async () => {
        ...
        await expect(
            makeBackend(fake).execute(request(), sessionContext()),
        ).rejects.toMatchObject({
            code: 'MICROVM_LAUNCH_FAILED',
        });
        const firstToken = (
            fake.callsFor('runMicrovm')[0].args as { clientToken?: string }
        ).clientToken;
    ...
    test('migrates a poisoned legacy PENDING token after a validation rejection', async () => {
        ...
        await writeRuntimeSessionRecord(
            {
                ...
                last_seen_at: launchedAt,
                hard_deadline_at:
                    launchedAt + newCfg.maxDurationSeconds * 1_000 - 60_000,
            },
            lock as string,
        );
        await releaseRuntimeSessionLock('rt_session_1', lock as string);
        ...
        await expect(
            makeBackend(fake, { imageVersion: '4' }).execute(
                request(),
                sessionContext(),
            ),
    ...
    test('uses a distinct launch token after registry loss and an image upgrade', async () => {
        ...
        const runCalls = fake.callsFor('runMicrovm');
        expect(runCalls).toHaveLength(2);
        const secondToken = (runCalls[1].args as { clientToken?: string })
            .clientToken;
        expect(secondToken).not.toBe(firstToken);
        expect(secondToken?.length).toBeLessThanOrEqual(128);
        expect(
            (await readRuntimeSessionRecord('rt_session_1'))?.image_version,
        ).toBe('4');
    });
    ...
    test('does not replay a persisted token when the exact connector request changed', async () => {
        ...
        await expect(
            makeBackend(fake, { egressConnectorArns: firstConnectors }).execute(
                request(),
                sessionContext(),
            ),
        ).rejects.toMatchObject({
        ...
        await expect(
            makeBackend(fake, {
                egressConnectorArns: [...firstConnectors].reverse(),
    ...
    test('terminalizes two known-dead launch tokens without losing the checkpoint pointer', async () => {
        ...
        await writeRuntimeSessionRecord(
            {
                ...
                workspace_checkpoint:
                    'sessions/rt_session_1/checkpoints/redis-only.gtar',
        ...
        expect(retired?.workspace_checkpoint).toBe(
            'sessions/rt_session_1/checkpoints/redis-only.gtar',
        );
        ...
        expect((await readRuntimeSessionRecord('rt_session_1'))?.state).toBe(
            'RUNNING',
        );
    });
    ...
    test('bounds an unstructured legacy validation fallback to one migrated token', async () => {
        ...
        fake.failNext(
            'runMicrovm',
            new LambdaMicrovmApiError('validation', 'RunMicrovm', 'bad role'),
        );
        fake.failNext(
            'runMicrovm',
            new LambdaMicrovmApiError('validation', 'RunMicrovm', 'bad role'),
        ...
        expect(migrated?.generation).toBeGreaterThanOrEqual(
            RUNTIME_SESSION_NAMESPACED_GENERATION_MIN,
        );
        ...
        const thirdToken = (
            fake.callsFor('runMicrovm')[2].args as { clientToken?: string }
        ).clientToken;
    ...
    test('a reused VM skips the preflight health check so a slow auto-resume can proceed', async () => {
        ...
        expect(captured.filter(c => c.path === '/api/v2/health')).toHaveLength(
            0,
        );
        expect(captured.filter(c => c.path === '/api/v2/execute')).toHaveLength(
            1,
        );
    });
    ...
    test('probes the VM and pushes only what it is missing', async () => {
        ...
        expect(paths.indexOf('/api/v2/session/inputs')).toBeLessThan(
            paths.indexOf('/api/v2/execute'),
        );
        const push = captured.find(
            request => request.path === '/api/v2/session/inputs',
        );
        ...
        const untarred = zlib
            .gunzipSync(lastSessionFilesBody!)
            .toString('latin1');
    ...
    test('a second execution pushes nothing when the VM already holds the object', async () => {
        ...
        expect(
            captured.filter(c => c.path === '/api/v2/session/inputs/probe'),
        ).toHaveLength(1);
        expect(
            captured.filter(c => c.path === '/api/v2/session/inputs'),
        ).toHaveLength(0);
    });
    ...
    test('a stateless one-shot receives its by-ref inputs too', async () => {
        ...
        const paths = captured.map(c => c.path);
        expect(paths.indexOf('/api/v2/session/inputs')).toBeGreaterThanOrEqual(
            0,
        );
        expect(paths.indexOf('/api/v2/session/inputs')).toBeLessThan(
            paths.indexOf('/api/v2/execute'),
    ...
    test('a live runner rejecting an input push keeps the warm session VM', async () => {
        ...
        await expect(
            backend.execute(request(), sessionContext()),
        ).rejects.toThrow('Session input push failed');
        expect(captured.filter(c => c.path === '/api/v2/execute')).toHaveLength(
            0,
        );
        expect(fake.callsFor('terminateMicrovm')).toHaveLength(0);
    ...
    test('rejects a probe response containing an unrequested cache key', async () => {
        ...
        await expect(
            makeBackend(fake).execute(request(), sessionContext()),
        ).rejects.toThrow('Session input delivery failed');
        expect(captured.filter(c => c.path === '/api/v2/execute')).toHaveLength(
            0,
        );
        expect(fake.callsFor('terminateMicrovm')).toHaveLength(0);
    ...
    test('re-probes the full working set after push and never executes after eviction', async () => {
        ...
        await expect(
            makeBackend(fake).execute(request(), sessionContext()),
        ).rejects.toThrow('working set');
        const paths = captured.map(c => c.path);
        const pushIndex = paths.indexOf('/api/v2/session/inputs');
        expect(pushIndex).toBeGreaterThanOrEqual(0);
        expect(
            paths.lastIndexOf('/api/v2/session/inputs/probe'),
        ).toBeGreaterThan(pushIndex);
    ...
    test('a payload with no by-ref inputs skips probe and push entirely', async () => {
        ...
        expect(
            captured.filter(c => c.path.startsWith('/api/v2/session/inputs')),
        ).toHaveLength(0);
    });
    ...
    test('lock contention is retryable BUSY, never a cold one-shot', async () => {
        ...
        for (const mode of ['affinity', 'strict'] as const) {
            await expect(
                backend.execute(
                    request(),
                    sessionContext({ runtimeSessionMode: mode }),
                ),
    ...
    test('relaunches an idle-expired session instead of reusing the dead endpoint', async () => {
        ...
        const token = (await acquireRuntimeSessionLock(
            'rt_session_1',
            60_000,
        )) as string;
        const rec = await readRuntimeSessionRecord('rt_session_1');
        await writeRuntimeSessionRecord({ ...rec!, last_seen_at: 1 }, token);
        const { releaseRuntimeSessionLock } =
            await import('../runtime-session/registry');
    ...
    test('strict mode raises RUNTIME_SESSION_BUSY when the lock is held', async () => {
        ...
        try {
            await backend.execute(
                request(),
                sessionContext({ runtimeSessionMode: 'strict' }),
            );
            throw new Error('expected rejection');
        } catch (error) {
            expect(error).toBeInstanceOf(SandboxBackendError);
    ...
    test('stateless mode ignores a runtime session id', async () => {
        ...
        await backend.execute(
            request(),
            sessionContext({ runtimeSessionMode: 'stateless' }),
        );
        const runArgs = fake.callsFor('runMicrovm')[0].args as {
            runHookPayload?: string;
        };
    ...
    test('session mode refuses an unpinned image version before launch', async () => {
        const fake = fakeClient();
        await expect(
            makeBackend(fake, { imageVersion: undefined }).execute(
                request(),
                sessionContext(),
            ),
    ...
    test('sends X-aws-proxy-port only when the port is not the 8080 default', async () => {
        const fake = fakeClient();
        await makeBackend(fake, { port: 9090 }).execute(
            request(),
            sessionContext(),
        );
        ...
        captured = [];
        await makeBackend(fake, { port: 8080 }).execute(
            request(),
            sessionContext({ runtimeSessionId: 'rt_8080' }),
        );
    ...
    test('terminates a superseded (config-drifted) VM before relaunching', async () => {
         ...
         * launches. */
        await makeBackend(fake, { imageVersion: '4' }).execute(
            request(),
            sessionContext(),
        );
        expect(fake.callsFor('runMicrovm')).toHaveLength(2);
        const terminated = fake
            .callsFor('terminateMicrovm')
            .map(c => (c.args as { microvmId: string }).microvmId);
    ...
    test('launch-record failure after RunMicrovm terminates the untracked VM', async () => {
        ...
        const originalWrite =
            redisWithScript.writeRuntimeSessionRecordScript.bind(mock);
        let writes = 0;
        ...
        await expect(
            makeBackend(fake).execute(request(), sessionContext()),
        ).rejects.toThrow('Lost session lock for rt_session_1 after launch');
        expect(fake.callsFor('runMicrovm')).toHaveLength(1);
        expect(fake.callsFor('terminateMicrovm')).toHaveLength(1);
    });
    ...
    test('a tightened egress connector config makes an existing session non-reusable', async () => {
        ...
        await makeBackend(fake, {
            egressConnectorArns: [
                'arn:aws:lambda:us-east-2:1:network-connector:vpc-egress',
            ],
        }).execute(request(), sessionContext());
        expect(fake.callsFor('runMicrovm')).toHaveLength(2);
        const terminated = fake
...
describe('LambdaMicrovmSandboxBackend auto-checkpoint', () => {
    ...
    const cfgOn: Partial<LambdaMicrovmBackendConfig> = {
    ...
    test('checkpoints the workspace after a session exec and records the pointer', async () => {
        ...
        const checkpoints = captured.filter(
            c => c.path === '/api/v2/session/checkpoint',
        );
        ...
        expect(checkpoints[0].headers['x-runtime-session-id']).toBe(
            'rt_ckpt_1',
        );
        const stored = await store.get('rt_ckpt_1', 1_000_000);
        expect(stored).not.toBeNull();
        try {
            expect(await fsp.readFile(stored!.path, 'utf8')).toBe(
                checkpointBlob,
            );
        } finally {
        ...
        expect(record?.workspace_checkpoint).toStartWith(
            checkpointPrefixFor('rt_ckpt_1'),
        );
    ...
    test('restores the prior checkpoint on retry when result finalization fails', async () => {
        ...
        expect(
            firstAttemptPaths.filter(
                path => path === '/api/v2/session/checkpoint',
            ),
        ).toHaveLength(0);
        expect(fake.callsFor('terminateMicrovm')).toHaveLength(1);
        ...
        const restoreIndexes = paths
            .map((path, index) =>
                path === '/api/v2/session/restore' ? index : -1,
            )
            .filter(index => index >= 0);
    ...
    test('still terminates the mutated VM when the rollback registry read fails', async () => {
        ...
        expect((await readRuntimeSessionRecord('rt_ckpt_1'))?.state).toBe(
            'RUNNING',
        );
    });
    ...
    test('still terminates the mutated VM when the rollback quarantine write fails', async () => {
        ...
        redisWithScript.writeRuntimeSessionRecordScript = async (
            ...
            if (failQuarantineWrite)
                throw new Error('registry write unavailable');
            return originalWrite(...args);
        };
        const sessionResultFinalizer = async (
            _result: SandboxRawResponse,
    ...
    test('a relaunched VM restores the checkpoint before the first exec', async () => {
        ...
        await writeRuntimeSessionRecord(
            {
                ...
                generation: 3,
                last_seen_at: 1,
                workspace_checkpoint: checkpointObjectKey('rt_ckpt_1', 1000),
            },
            seedToken as string,
        );
        const { releaseRuntimeSessionLock } =
        ...
        expect(paths.indexOf('/api/v2/session/restore')).toBeGreaterThanOrEqual(
            0,
        );
        expect(paths.indexOf('/api/v2/session/restore')).toBeLessThan(
            paths.indexOf('/api/v2/execute'),
        );
        const restoreReq = captured.find(
            c => c.path === '/api/v2/session/restore',
        );
    ...
    test('reuse (warm VM) does not restore — no prior expiry', async () => {
        ...
        expect(
            captured.filter(c => c.path === '/api/v2/session/restore'),
        ).toHaveLength(0);
        expect(fake.callsFor('runMicrovm')).toHaveLength(1);
    });
    ...
    test('disabled checkpoints skip both checkpoint and restore', async () => {
        ...
        expect(
            captured.filter(
                c =>
                    c.path === '/api/v2/session/checkpoint' ||
                    c.path === '/api/v2/session/restore',
            ),
        ).toHaveLength(0);
    ...
    test('skips a checkpoint unless the budget covers the complete bounded pipeline', async () => {
        ...
        try {
            ...
            expect(await backend.execute(request(), sessionContext())).toEqual(
                EXECUTE_RESPONSE,
            );
            expect(
                captured.filter(c => c.path === '/api/v2/session/checkpoint'),
            ).toHaveLength(0);
            expect(store.objects.size).toBe(0);
        } finally {
    ...
    test('shares one absolute token deadline across checkpoint throttle and SDK legs', async () => {
        ...
        fake.createMicrovmAuthToken = async (args, signal?: AbortSignal) => {
            ...
            return new Promise((_, reject) => {
                if (!signal) {
                    reject(
                        new Error(
                            'checkpoint token mint omitted its deadline signal',
                        ),
                    );
                    return;
                }
                const onAbort = (): void =>
                    reject(signal.reason ?? new Error('token mint aborted'));
        ...
        onExecute = () =>
            mock
                .set('rtsx:tps:poison:token', '1', 'PX', 80)
                .then(() => undefined);
        const store = new MemoryCheckpointStore();
        const backend = makeBackend(
            fake,
    ...
    test('aborts a slow-trickle guest checkpoint on the absolute transfer deadline', async () => {
        ...
        expect(await backend.execute(req, sessionContext())).toEqual(
            EXECUTE_RESPONSE,
        );
        expect(
            captured.filter(c => c.path === '/api/v2/session/checkpoint'),
    ...
    test('a checkpoint FETCH failure fails closed instead of running on an empty workspace', async () => {
        ...
        await expect(
            backend.execute(request(), sessionContext()),
        ).rejects.toThrow('refusing to run against an empty workspace');
        expect(captured.filter(c => c.path === '/api/v2/execute')).toHaveLength(
            0,
        );
        expect(fake.callsFor('terminateMicrovm')).toHaveLength(1);
    ...
    test('a failed first reseed lookup cannot strand the Redis sequence below retained objects', async () => {
        ...
        const record = await readRuntimeSessionRecord('rt_ckpt_1');
        expect(record?.workspace_checkpoint).toBe(
            checkpointObjectKey('rt_ckpt_1', 101),
        );
        expect(store.objects.has(checkpointObjectKey('rt_ckpt_1', 101))).toBe(
            true,
        );
    });
    ...
    test('a commit-marker failure keeps the previous durable recovery point', async () => {
        ...
        expect(
            (await readRuntimeSessionRecord('rt_ckpt_1'))?.workspace_checkpoint,
        ).toBe(checkpointObjectKey('rt_ckpt_1', 11));
        expect(pruneCalls).toBe(0);

        const durable = await store.get('rt_ckpt_1', 1_000_000);
        try {
            expect(await fsp.readFile(durable!.path, 'utf8')).toBe(
                'PRIOR_WORKSPACE',
            );
        } finally {
