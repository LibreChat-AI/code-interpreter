...
function requireValue(name: string, value: string | undefined): void {
    if (!nonEmpty(value)) {
        throw new SecureStartupConfigError(
            `${name} is required in CODEAPI_HARDENED_SANDBOX_MODE`,
        );
    }
}
...
function requireStrongSecret(
    ...
    if (Buffer.byteLength(value ?? '', 'utf8') < minBytes) {
        throw new SecureStartupConfigError(
            `${name} must be at least ${minBytes} bytes in CODEAPI_HARDENED_SANDBOX_MODE`,
        );
    }
}
...
function rejectValue(name: string, value: string | undefined): void {
    if (nonEmpty(value)) {
        throw new SecureStartupConfigError(
            `${name} must not be configured in CODEAPI_HARDENED_SANDBOX_MODE`,
        );
    }
}

function requireSafeWholeNumber(
    name: string,
    value: number,
    min: number,
): void {
...
export function validateWorkerHardenedConfig(): void {
    ...
    rejectValue(
        ...
        process.env.CODEAPI_SYNTHETIC_ACCESS_TOKEN,
    );
    requireValue('EGRESS_GATEWAY_URL', env.EGRESS_GATEWAY_URL);
    requireValue(
        INTERNAL_SERVICE_TOKEN_ENV,
...
export function validateSandboxBackendPolicy(): void {
    ...
    if (env.LAMBDA_MICROVM_IMAGE_ARN.trim().length === 0) {
        throw new SecureStartupConfigError(
            'LAMBDA_MICROVM_IMAGE_ARN is required for the lambda-microvm backend',
        );
    }
    if (
        env.RUNTIME_SESSION_MODE !== 'stateless' &&
    ...
    if (
        ...
        (env.LAMBDA_MICROVM_EGRESS_CONNECTOR_ARNS?.length ?? 0) === 0
    ) {
        throw new SecureStartupConfigError(
    ...
    if (
        env.HARDENED_SANDBOX_MODE &&
        (env.LAMBDA_MICROVM_EGRESS_CONNECTOR_ARNS?.some(arn =>
            arn.trim().endsWith(AWS_MANAGED_INTERNET_EGRESS_SUFFIX),
        ) ??
            false)
    ) {
        throw new SecureStartupConfigError(
    ...
    if (
        env.LAMBDA_MICROVM_ALLOW_SHELL &&
        (env.HARDENED_SANDBOX_MODE || process.env.NODE_ENV === 'production')
    ) {
        throw new SecureStartupConfigError(
    ...
    if (env.SESSION_CHECKPOINTS && env.RUNTIME_SESSION_MODE !== 'stateless') {
        ...
        if (hasAccessKey !== hasSecretKey) {
            missing.push(
                hasAccessKey ? 'MINIO_SECRET_KEY' : 'MINIO_ACCESS_KEY',
            );
        }
        if (
            !nonEmpty(process.env.CODEAPI_CHECKPOINT_BUCKET) &&
            !nonEmpty(process.env.MINIO_BUCKET)
        ) {
...
export function validateEgressGatewayHardenedConfig(): void {
    ...
    if (!env.EGRESS_LEDGER_REQUIRED) {
        throw new SecureStartupConfigError(
            'CODEAPI_EGRESS_LEDGER_REQUIRED must be true in CODEAPI_HARDENED_SANDBOX_MODE',
        );
    }
}
