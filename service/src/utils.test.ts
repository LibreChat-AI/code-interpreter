...
describe('sandbox error formatting', () => {
    ...
    test('maps sandbox setup failures from worker errors to public responses', () => {
        const err = new Error(
            'Error from sandbox [mount_failed]: Sandbox setup failed: workspace mount failed',
        );
    ...
    test('maps a busy runtime session (strict mode) to 409', () => {
        const err = new Error(
            'RUNTIME_SESSION_BUSY: Runtime session rt_abc is busy',
        );
        expect(publicExecutionFailure(err)).toEqual({
            status: 409,
            body: {
                error: 'runtime_session_busy',
                message: 'Runtime session is busy',
            },
        });
    });
    ...
    test('maps MicroVM launch failures to 503', () => {
        const err = new Error(
            'MICROVM_LAUNCH_FAILED: MicroVM did not reach RUNNING within 60000ms',
        );
        expect(publicExecutionFailure(err)).toEqual({
            status: 503,
            body: {
                error: 'microvm_launch_failed',
    ...
    test('maps a recycled dirty session to a retryable public failure', () => {
        const failure = publicExecutionFailure(
            new Error(
                'MICROVM_UNHEALTHY: Runtime session rt_private workspace was dirty and has been recycled',
            ),
        );
    ...
    test('maps oversized input delivery to 413', () => {
        const err = new Error(
            'SESSION_INPUT_TOO_LARGE: Session inputs exceed the 536870912-byte budget',
        );
        expect(publicExecutionFailure(err)).toEqual({
            status: 413,
            body: {
    ...
    test('maps unavailable input objects to 422 without exposing object details', () => {
        const failure = publicExecutionFailure(
            new Error(
                'SESSION_INPUT_UNAVAILABLE: Failed to fetch private/customer-list.csv from file-server.internal',
            ),
        );
        expect(failure).toEqual({
            status: 422,
    ...
    test('maps input source outages to 502 rather than MicroVM unavailability', () => {
        const err = new Error(
            'SESSION_INPUT_SOURCE_FAILED: upstream request failed with status 503',
        );
        expect(publicExecutionFailure(err)).toEqual({
            status: 502,
            body: {
    ...
    test('maps an input-delivery deadline to a sanitized 504', () => {
        const failure = publicExecutionFailure(
            new Error(
                'SESSION_INPUT_ABORTED: Fetching private/customer-list.csv timed out',
            ),
        );
        expect(failure).toEqual({
            status: 504,
    ...
    test('maps worker and BullMQ deadline fallbacks to sanitized 504s', () => {
        expect(
            publicExecutionFailure(new Error('Job timed out after 300000ms')),
        ).toEqual({
            status: 504,
            body: {
        ...
        expect(JSON.stringify(bullmqFailure)).not.toContain(
            'private-session-id',
        );
    });
    ...
    test('does not expose AWS identifiers embedded in backend failures', () => {
        ...
        const failure = publicExecutionFailure(
            new Error(
                `MICROVM_LAUNCH_FAILED: Access denied while passing ${arn} to mvm-secret-123`,
            ),
        );
        expect(failure).toEqual({
            status: 503,
    ...
    test('maps sandbox request guard failures to public bad requests', () => {
        const axiosErr = {
            ...
            response: {
                ...
                data: {
                    message:
                        'run_timeout cannot exceed the configured limit of 15000',
                },
            },
        } as AxiosError;

        const workerErr = new Error(
            `Error from sandbox: ${sandboxErrorMessageFromAxios(axiosErr)}`,
        );
    ...
    test('maps sanitized sandbox execution failures to bad gateway', () => {
        const err = new Error(
            'Error from sandbox [sandbox_execution_failed]: Sandbox execution failed',
        );
    ...
    test('does not expose arbitrary sandbox errors as public infrastructure failures', () => {
        const err = new Error(
            'Error from sandbox [unexpected]: very specific internal detail',
        );
