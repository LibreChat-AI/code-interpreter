...
export function filterSystemLogs(stderr: string, isPyPlot?: boolean): string {
    ...
    const logPattern =
        /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2},\d{3} - (INFO|WARNING|ERROR|CRITICAL) - /;
...
function parseErrorResponseData(data: unknown): {
    error?: string;
    message?: string;
} {
...
export function publicExecutionFailure(
    ...
    const bullmqWaitExpired =
        /^Job wait .+ timed out before finishing, no finish notification arrived after \d+ms \(id=.+\)$/.test(
            message,
        );
    ...
    if (backendMatch) {
        ...
        const publicMessages: Record<string, string> = {
            ...
            SESSION_INPUT_PREPARATION_FAILED:
                'Input files could not be prepared',
    ...
    const match = message.match(
        /^Error from sandbox(?:\s+\[([a-z_]+)\])?:\s*(?:\[([a-z_]+)\]\s*)?(.+)$/,
    );
